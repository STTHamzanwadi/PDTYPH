...
public class DaftarAdmin extends Panel implements View{
	/**
	 * 
	 */
	private static final long serialVersionUID = -8265667895917038873L;
	private Table tabel = new Table("DAFTAR ADMIN YAYASAN YPHPPD NW PANCOR");
	private VerticalLayout dashboardPanels;
	private Label titleLabel;
	private VerticalLayout root;
	private BeanContainer<Integer, Admin> beans = new BeanContainer<Integer, Admin>(Admin.class);
	...
	public DaftarAdmin() {
		...
		tambah.addClickListener(new ClickListener() {
			@Override public void buttonClick(ClickEvent event) {
				tambahUserYayasanbaru(new Admin());
			}
	...
	private Component getTable(){
		List<Admin> lm =GenericPersistence.findList(Admin.class);
	...
	private void tambahUserYayasanbaru(Admin i) {
		final Window win = new Window("Tambah Admin Baru");
		Component c = new TambahAdmin(i,win);
